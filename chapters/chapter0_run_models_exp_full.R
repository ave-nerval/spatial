...
formulaB.S.Exp.prior.small = last004scaled ~ 1 + vpliv1 + vpliv2 + vpliv3  +  
...
formulaB.HS.Exp.prior.small <- last004scaled ~ 1 + vpliv1 + vpliv2 + vpliv3 + 
  f(rowNumberAinv, model = "generic0", Cmatrix = Ainv,hyper = hyperVarPed) + 
...
model.Exp.S.prior.small.NA.full <- inla(formulaB.S.Exp.prior.small,  family = "gaussian", 
                                   data = phenoData.small, control.compute = list(dic=T,cpo=F,config=T), verbose=T)
model.Exp.HS.prior.small.NA.full <- inla(formulaB.HS.Exp.prior.small,  family = "gaussian", 
                                    data = phenoData.small, control.compute = list(dic=T,cpo=F,config=T), verbose=T)
...
save.image("C:/Users/JerryBond/Desktop/Eva/2022/data/workspaces/20220407_models_exp_pred_full.RData")
