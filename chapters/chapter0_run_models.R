...
phenoData.small$last004scaledNA <- phenoData.small$last004scaled
phenoData.small$last004scaledNA[year_pred] <- NA # before 50, now 292 missing values

sum(is.na(phenoData.small$last004scaledNA)) # 292, OK
...
formulaS.1 = 'last004scaledNA ~ intercept + f(rowNumberAinv, model = "generic0", Cmatrix = Ainv,hyper = hyperVarPed) + f(fieldID, model = spdeStatS) -1'

formulaHS.1 = 'last004scaledNA ~intercept + f(rowNumberAinv, model = "generic0", Cmatrix = Ainv,hyper = hyperVarPed) + f(idlok, model = "iid", hyper = hyperVarIdlok.GHS) + f(fieldID, model = spdeStatHS) -1'
...
# Make stack
stack = inla.stack(data = list(last004scaledNA = phenoData.small$last004scaledNA),
                   A = list(A,1, 1),
...
stackHS = inla.stack(data = list(last004scaledNA = phenoData.small$last004scaledNA),
                     A = list(A,1, 1),
...
formulaB.H = last004scaledNA ~ 1 + vpliv1 + vpliv2 + vpliv3  +  
  f(rowNumberAinv, model = "generic0", Cmatrix = Ainv,hyper = hyperVarPed) + 
...
formulaB.HS.OB.C <- last004scaledNA ~ 1 + vpliv1 + vpliv2 + vpliv3 + 
  f(rowNumberAinv, model = "generic0", Cmatrix = Ainv,hyper = hyperVarPed) + 
  f(idlok, model = "iid", hyper = hyperVarIdlok.GHS) + 
...
model.G.small.NA <- inla(formulaB.G,  family = "gaussian", data = phenoData.small, 
                         control.compute = list(dic=T,cpo=F, openmp.strategy="huge", config=T), verbose=T)
model.H.small.NA <- inla(formulaB.H,  family = "gaussian", data = phenoData.small, 
                         control.compute = list(dic=T,cpo=F, openmp.strategy="huge", config=T), verbose=T)

model.besag.S.UE.small.C.NA <- inla(formulaB.S.UE.C,  family = "gaussian", data = phenoData.small, 
                                    control.compute = list(dic=T,cpo=F, openmp.strategy="huge", config=T), verbose=T)

model.besag.HS.UE.small.C.NA <- inla(formulaB.HS.UE.C,  family = "gaussian", data = phenoData.small, 
                                     control.compute = list(dic=T,cpo=F, openmp.strategy="huge", config=T), verbose=T)

model.besag.HS.UE.small.C.NA_full <- inla(formulaB.HS.UE.C_full,  family = "gaussian", data = phenoData.small, 
                                     control.compute = list(dic=T,cpo=F, openmp.strategy="huge", config=T), verbose=T)


model.besag.S.OB.small.C.NA <- inla(formulaB.S.OB.C,  family = "gaussian", data = phenoData.small, 
                                    control.compute = list(dic=T,cpo=F, openmp.strategy="huge", config=T), verbose=T)

model.besag.HS.OB.small.C.NA <- inla(formulaB.HS.OB.C,  family = "gaussian", data = phenoData.small, 
                                     control.compute = list(dic=T,cpo=F, openmp.strategy="huge", config=T), verbose=T)

# SPDE
model.SPDE.S.small.NA = inla(formula = formulaS, data = inla.stack.data(stack),
...
model.SPDE.HS.small.NA2 = inla(formula = formulaHS, data = inla.stack.data(stackHS),
